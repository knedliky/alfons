<script lang="ts">
	 ...
	 *
	 * The deck never empties: with nothing running it holds one card, the
	 * contents card, and this is what the contents card contains. The card's
	 * role does not change — it is still "the whole" — only the whole is now
	 * nothing, and that sentence is a complete answer. The alternative, a
	 * contents card reading "no runners" ahead of a separate card saying so
	 * again, is two cards for one fact.
	 *
	 * The page keeps the same shape it has when four things are running, so
...
<style>
	/* No background, border or padding of its own: this is the content of the
	   contents card, not a card beside it. The surface belongs to the card. */
	.empty {
		flex: 1;
		min-height: 0;
		display: flex;
