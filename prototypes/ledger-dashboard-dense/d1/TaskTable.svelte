...
<script lang="ts">
	 ...
	 * is in this release".
	 *
	 * Every type property in this file comes from a --ledger-rung-* custom
	 * property declared on the page root. Nothing here sets a family, size,
	 * weight or ink of its own. That is deliberate: the ladder is the variable
	 * this approach is testing, and a rung set locally would be a second
	 * declaration nobody would find again.
	 */
...
<style>
	...
	/*
		Rung 3 is the table's resting voice: any cell that claims nothing else
		is a classifier. Rungs 1, 2 and 4 are then opted into per column, below.
	*/
	table {
		width: 100%;
		...
		font-weight: var(--ledger-rung-3-weight);
		letter-spacing: var(--ledger-rung-3-tracking);
	...
	/*
		The head sits at rung 4 rather than above the table, because a column
		name is read once and a column of values is read eighty-seven times.
		Sorting is what promotes a header: the active one moves to rung 2, which
		is the only place on the page a rung is used to say "this one", and it
		is why the sort state is legible without the arrow.
	*/
	.sort {
		display: flex;
		...
		font-weight: var(--ledger-rung-4-weight);
	...
	/*
		The banner runs the same ladder one notch up: the slug is the release's
		id, so it takes rung 2's face at rung 1's ink, and the release title
		takes rung 1's size in the display face. A banner that used a fifth,
		larger set of steps would have made the band a heading rather than a
		row, and the band is a row.
	*/
	.banner-slug {
	...
	.banner-title {
		...
		font-size: var(--ledger-rung-1-size);
	...
	/*
		Mixing sizes across a row is what a ladder costs, and this is the price.
		Under vertical-align: middle — a4's setting, correct when every cell is
		one size — each cell centres on its own cap height, so a row carrying
		three sizes has a ragged top edge, which is exactly the quality a dense
		table lives on. Aligning baselines instead puts all four rungs on one
		line and pushes the raggedness down into the descenders, where nobody
		reads it. This is the only rule in the file that changed for a reason
		other than the ladder itself, and it changed because of the ladder.
	*/
	tbody td {
	...
	/*
		...
		Every column claims exactly one rung, and this block is the whole claim.
		The test the assignment had to pass: a column belongs one rung higher
		than another only if you would reach for it earlier while hunting for a
		half-remembered task. Title first, then the strings you would say aloud,
		then the words that narrow a set, then the tallies you only read once
		the row is already found.

		Rung 1  title
		Rung 2  id, release, status word (in StatusMark)
		Rung 3  project, type, risk, verdict — table default, set above
		Rung 4  phase, steps, criteria, files, attempt, created, sealed
	*/

	/* Rung 1 — the thing being recognised. The only cell in the body face and
	   the only cell at primary ink, on the whole page. Absorbing the slack and
	   truncating keeps every other column on its own axis. */
	.col-title {
		width: 100%;
		max-width: 0;
		overflow: hidden;
	...
	.title {
		font-family: var(--ledger-rung-1-family);
		font-size: var(--ledger-rung-1-size);
		font-weight: var(--ledger-rung-1-weight);
		letter-spacing: var(--ledger-rung-1-tracking);
		color: var(--ledger-rung-1-ink);
	}

	/* Rung 2 — the locators. An id and a release slug are the two strings that
	   name the work out loud, and they are the two the eye returns to after the
	   title has been matched, so they are the only mono cells above the field. */
	.task-id,
	.col-release {
		font-size: var(--ledger-rung-2-size);
		font-weight: var(--ledger-rung-2-weight);
	...
	/* Rung 3 — the classifiers. They inherit the table's resting voice and need
	   only their ink stated, which is the point of making rung 3 the default. */
	.col-project,
	.col-type,
	.col-verdict {
		color: var(--ledger-rung-3-ink);
	...
	/*
		Rung 4 — the substrate. Same face and same size as rung 3 on purpose:
		this is the step that could not be taken by size, so it is taken by ink
		and weight alone. It still separates, because seven adjacent columns all
		dropping to muted at once reads as a band rather than as seven cells.
	*/
	.col-phase,
	.col-steps,
	.col-criteria,
	...
	.col-sealed {
		font-weight: var(--ledger-rung-4-weight);
		letter-spacing: var(--ledger-rung-4-tracking);
		color: var(--ledger-rung-4-ink);
	}
	...
	/* Risk and a failed verdict are the two rung-3 cells allowed to borrow rung
	   1's ink. They are borrowing ink and not size, so the row's rhythm is
	   untouched — which is the whole reason ink is the cheapest rung dimension
	   and size the dearest. */
	.risk[data-risk='high'],
	...
	/* The detail strip runs the same four rungs, so the page has one ladder and
	   not two: block headings at rung 4, ids at rung 2, sibling titles at
	   rung 1. A strip with its own scale would have read as a different page
	   opening underneath the row. */
	.detail-block h3 {
	...
	.detail-title {
		...
		font-size: var(--ledger-rung-1-size);
		font-weight: var(--ledger-rung-1-weight);
		/* Rung 1's face and size at rung 2's ink: a sibling is a title you are
		   reading, not the title you were looking for. */
		color: var(--ledger-rung-2-ink);
	}
