...
<div class="bar">
	...
	<span class="track" role="img" aria-label="Horizontal position in the metadata columns">
		<span
			...
			style:left="{scrollMax > 0
				? (scrollLeft / scrollMax) * (100 - (viewport / scrollWidth) * 100)
				: 0}%"
		></span>
	</span>
...
<style>
	...
	*/
	...
	@keyframes settle {
		from {
			background:
				linear-gradient(var(--accent-bg-emphasis), var(--accent-bg-emphasis)), var(--bg-glass-solid);
		}
		to {
			background: var(--bg-glass-solid);
		}
