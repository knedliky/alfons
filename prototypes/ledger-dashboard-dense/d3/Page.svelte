<script lang="ts">
	 ...
	 *
	 * Direction: freeze an identity spine — status, id, title — and let the
	 * twelve metadata columns scroll horizontally underneath it. The winning
	 * approach of round one named its own worst failure: below about 1200px it
	 * scrolls sideways rather than dropping columns, deliberately, because a
	 * hidden column defeats a density study. This approach takes that constraint
	 * as given and attacks the consequence instead. Every column survives at
	 * every width. What changes is that horizontal scrolling stops costing you
	 * the row you were reading.
	 *
	 * The contrast device is therefore structural rather than typographic: two
	 * planes, one fixed and one moving, with the seam between them the single
	 * most carefully drawn line on the page.
	 */
	import {
		Container,
		Footer,
	...
	<main class="ledger">
		<PageSection maxWidth="1680px" class="band band-masthead">
			<Container maxWidth="full" padding={false}>
				...
				<PageHeader
					title="Fifteen columns, three of them nailed down"
					subtitle="Status, id and title stay where they are. The other twelve scroll underneath them, and nothing is dropped at any width."
					align="left"
					spacing="none"
				/>
			...
			<Container maxWidth="full" padding={false}>
				<div class="legend">
					<div class="legend-block">
						...
						<p class="legend-note">
							Status sits in the frozen spine, so it is one of only three things always on screen. It
							carries a glyph and a word as well as its colour, which is what lets it survive that
							weight.
						</p>
					</div>

					<div class="legend-block">
						<h2>Reading the spine</h2>
						<dl class="legend-keys">
							<div>
								<dt>Seam</dt>
								<dd>
									a plain column rule at rest; it brightens and casts only once something is
									genuinely passing underneath
								</dd>
							</div>
							<div>
								<dt>Track</dt>
								<dd>the share of the twelve scrolling columns currently on screen</dd>
							</div>
							<div>
								<dt>Keys</dt>
								<dd>
									tab into the rail, then arrows to scroll either way, Home and End for the two
									ends
								</dd>
							</div>
							<div>
								<dt>Ph · Stp · Crt</dt>
								<dd>phase, steps, criteria</dd>
							</div>
							<div>
								<dt>Fls · Att</dt>
								<dd>file changes, latest verification attempt</dd>
							</div>
						</dl>
						<p class="legend-note">
