<script lang="ts" module>
	...
	export interface TaskTableProps {
		...
		/** Every task in the corpus, for resolving siblings, dependencies and meters. */
		allTasks: Task[];
	}
	...
	const COLUMNS: ColumnSpec[] = [
		...
		{ id: 'trail', header: 'Trail', fullName: 'Verification trail' },
...
<script lang="ts">
	/**
	 * TaskTable — the whole corpus as one table, with data ink in two places.
	 *
	 ...
	 * row. All four are load-bearing here: status needs a glyph, the trail is a
	 * mark rather than a string, this is a public surface, and the release
	 * banner is the whole answer to "what else is in this release".
	 *
	 * The two graphical additions over round one are the release band's phase
	 * meter and the row's verification trail. Both sit beside the text they
	 * summarise rather than replacing it, so the page never depends on a reader
	 * decoding a mark four pixels wide.
	 */
	...
	/**
	 * The status breakdown each phase meter draws. Built from every task in the
	 * corpus, not from the filtered rows: a meter that narrowed with the search
	 * would stop being the release's shape and start being the search's.
	 */
	const tallyPerRelease = $derived.by(() => {
...
<div class="rail">
	<table>
		<caption class="visually-hidden">
			Every task in the corpus. Column headers sort the table in place. Each release band carries a
			stacked status meter, and each row a verification trail; both are also stated in words in the
			columns beside them.
		</caption>
		<thead>
			<tr>
		...
		<tbody>
			{#each rows as task (task.id)}
					...
					<tr class="banner">
						<th scope="colgroup" colspan={TOTAL_COLUMNS}>
							<div class="banner-inner">
								...
								<span class="banner-meter">
									<PhaseMeter
										counts={tallyPerRelease[banner.slug] ?? EMPTY_TALLY}
										label={banner.slug}
									/>
								</span>
							</div>
				...
				>
					...
					<td class="col-trail"><VerificationTrail attempts={task.attempts} /></td>
							...
							<div class="detail-inner">
									...
									<ul>
										{#each siblingsOf(task) as sibling (sibling.id)}
											<li class:same-phase={sibling.phase === task.phase}>
												...
												<VerificationTrail attempts={sibling.attempts} />
											</li>
										{/each}
									</ul>
								</section>
								...
								<section class="detail-block">
									...
									{#if task.attempts.length === 0}
										<p class="detail-empty">Never verified. No attempt has been recorded.</p>
									{:else}
										<ol class="history">
											{#each task.attempts as outcome, index (index)}
												<li>
													<span class="history-index figure">{index + 1}</span>
													<span class="history-outcome" data-verdict={outcome}>{outcome}</span>
												</li>
											{/each}
										</ol>
										<p class="detail-empty">
											{task.attempts.length}
											{task.attempts.length === 1 ? 'attempt' : 'attempts'}{task.latestSealedOn
												? `, last sealed ${task.latestSealedOn}`
												: ', none sealed'}.
										</p>
									{/if}
								</section>
...
<style>
	...
	/* Wide enough that the smallest segment in a nine-task release is still a
	   readable length rather than a dot. */
	.banner-meter {
		flex: none;
	...
	/* The trail sits immediately before the attempt count and the verdict it
	   summarises, so the mark and its words are read in one movement. */
	.col-trail {
	...
	/* The trail written out. Every mark on this page has one of these
	   somewhere, which is what makes the marks legal rather than decorative. */
	.history-index {
	...
	.history-outcome[data-verdict='pass'] {
		color: var(--text-primary);
	}
