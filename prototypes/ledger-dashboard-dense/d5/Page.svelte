<script lang="ts">
	...
	const releaseShapes = $derived.by(() => {
		...
		return RELEASES.map((release) => {
			...
			(a, b) => b.blocked - a.blocked || b.inFlight - a.inFlight || b.open - a.open
		);
	});
			...
			<Container maxWidth="full" padding={false}>
				<div class="legend">
					...
					<div class="legend-block">
						...
						<p class="legend-note">
							Both marks are stated in words as well: the meter in its accessible label, the trail in
							the Att and Verdict columns beside it and in full under the expanded row.
						</p>
					</div>
