<script lang="ts">
	...
	// takes focus when the camera arrives. SvelteMap keeps mutations compatible
	// with Svelte's ownership model even though it is read only by event handlers.
	const stops = new SvelteMap<string, HTMLElement>();
				...
				{:else}
					...
					<h1 class="headline">
						<button
							...
							class="lift"
							onclick={home}
							aria-label="{headline}. Lift the camera to the whole map."
						>
							{headline}
						</button>
					</h1>
