 ...
 *
 * Carried from the winning approach. In this approach the corpus is not a set
 * of destinations behind taps — it is the surface itself. The release level IS
 * this array walked left to right, and the project level is the projects
 * array, so the ORDER of these lists is part of the design: it is what a
 * horizontal swipe at each level moves through.
 *
 * A release panel shows a release, which means it shows tasks that are not
 ...
 * (building and verifying only) is a rule about the task level, not about the
 * levels above it.
 */
...
export const projects: Project[] = [
	{ name: 'alfons', blurb: 'The design system: tokens, components, and three surfaces over one manifest.' },
	{ name: 'atlas', blurb: 'The site and its local edge — Caddy, mounts and the public routes.' },
	{ name: 'ledger', blurb: 'The MCP server that is the single write path into the context database.' },
...
/**
 * Sibling positions on the two upper axes. -1 for a name the corpus does not
 * know, so a caller can keep its current position rather than jumping to 0 on
 * a typo'd hash.
 */
export function releaseIndexOf(slug: string): number {
	return releases.findIndex((release) => release.slug === slug);
}

export function projectIndexOf(name: string): number {
	return projects.findIndex((project) => project.name === name);
}

/** A task is settled once it is no longer in motion. Settled work renders at
 *  reduced strength on the upper levels — the u5 finding this round inherits. */
export function isSettled(status: ReleaseTask['status']): boolean {
