...
<style>
	...
	.scale :global(.toggle-btn) {
		flex: 1;
		/* Was --filter-control-height, inherited from the approach this builds on.
		   Measured this round: that token is 2.75rem on a phone and 2.25rem from
		   640px up, so the segments came out 41px at 1280px — under the touch
		   minimum at exactly the width where a touchscreen laptop reads this.
		   --space-7 is 48px at every width. Reported. */
