...
export type TaskStatus =
	'pending' | 'triaged' | 'building' | 'verifying' | 'blocked' | 'done' | 'abandoned';
...
export const projects: Project[] = [
	{
		...
		blurb:
			'One design system, three surfaces — library, Storybook, and the MCP server agents build against.'
	},
	{
		name: 'ledger',
...
export const releases: Release[] = [
	{
		...
		tasks: [
			{
				id: 'AL-011',
				title: 'The /dev surface: rounds discovered from the tree, not a registry',
				phase: 1,
				status: 'done'
			},
			{
				id: 'AL-012',
				title: 'The approach pager and the live work glow',
				phase: 1,
				status: 'done'
			},
			{
				id: 'AL-013',
				title: 'plan_prototype_round: write a round.json and seed five approach shells',
				phase: 2,
				status: 'done'
			},
			{
	...
	},
	{
		...
		tasks: [
			{
				id: 'AL-001',
				title: 'Token manifest generated from the CSS rather than curated by hand',
				phase: 1,
				status: 'done'
			},
			{
				id: 'AL-002',
				title: 'find_components: search the library by what it should do',
				phase: 1,
				status: 'done'
			},
			{
				id: 'AL-003',
				title: 'get_component: props, variants, snippets and the Storybook link',
				phase: 1,
				status: 'done'
			},
			{
				id: 'AL-004',
				title: 'review_markup: the design rules, as findings rather than a gate',
				phase: 2,
				status: 'done'
			},
			{
				id: 'AL-005',
				title: 'get_tokens narrowed by surface, with deprecations carrying replacements',
				phase: 2,
				status: 'done'
			},
			{
				id: 'AL-006',
				title: 'scaffold_component and the rules it writes against',
				phase: 2,
				status: 'done'
			},
			{
				id: 'AL-007',
				title: 'get_layout_recipe: the composition order, by tier',
				phase: 3,
				status: 'done'
			},
			{
				id: 'AL-008',
				title: 'review_library: sweep the whole library on the same rules',
				phase: 3,
				status: 'done'
			},
			{
				id: 'AL-009',
				title: 'apply_fixes: take a finding’s own correction',
				phase: 3,
				status: 'done'
			},
			{
				id: 'AL-010',
				title: 'Retire the curated token list and read Postgres instead',
				phase: 3,
				status: 'done'
			},
			{
				id: 'AL-015',
				title: 'Storybook catalogue published behind the proxy',
				phase: 4,
				status: 'done'
			},
			{
				id: 'AL-016',
				title: 'Lifecycle: retired components answer with their replacement',
				phase: 4,
				status: 'done'
			}
		]
	},
	{
		...
		tasks: [
			{
				id: 'LDG-039',
				title: 'Move schema/ out of the repository being dismantled',
				phase: 1,
				status: 'done'
			},
			{
				id: 'LDG-040',
				title: 'base.manifest: record what base.sql already contains',
				phase: 1,
				status: 'done'
			},
			{
			...
			{
				id: 'LDG-042',
				title: 'Emit CREATE EXTENSION lines pg_dump leaves out of a schema-scoped dump',
				phase: 2,
				status: 'triaged'
			},
			{
				id: 'LDG-043',
				title: 'Compare trigger definitions with search_path emptied on both sides',
				phase: 2,
				status: 'pending'
			},
			{
				id: 'LDG-044',
				title: 'Seed a CI corpus so the database-marked tests run rather than skip',
				phase: 3,
				status: 'pending'
			}
		]
	},
	{
		...
		tasks: [
			{
				id: 'LDG-035',
				title: 'Probe psql-18: bare psql resolves only in interactive shells',
				phase: 1,
				status: 'done'
			},
			{
				id: 'LDG-036',
				title: 'Scope the rule to the repository and say when the server has drifted',
				phase: 1,
				status: 'done'
			},
			{
				id: 'LDG-037',
				title: 'Collapse four near-identical hooks into one checked script',
				phase: 2,
				status: 'done'
			},
	...
	},
	{
		...
		tasks: [
			{
				id: 'ATL-115',
				title: 'A local domain per service rather than a remembered port',
				phase: 1,
				status: 'done'
			},
			{
				id: 'ATL-116',
				title: 'Certificates that the browser and curl both accept',
				phase: 1,
				status: 'done'
			},
			{
				id: 'ATL-117',
				title: 'Restart policy: bring the vite dev server back with the machine',
				phase: 2,
				status: 'done'
			},
			{
			...
			{
				id: 'ATL-119',
				title: 'Health check the proxy rather than the process behind it',
				phase: 3,
				status: 'blocked'
			}
		]
	},
	{
		...
		tasks: [
			{
				id: 'GW-070',
				title: 'Distinguish an expired token from a revoked one at the edge',
				phase: 1,
				status: 'done'
			},
			{
				id: 'GW-071',
				title: 'Carry the failure reason into the response body, not only the status',
				phase: 1,
				status: 'done'
			},
			{
			...
			{
				id: 'GW-073',
				title: 'Back off rather than hammer when the identity provider is down',
				phase: 2,
				status: 'triaged'
			}
		]
