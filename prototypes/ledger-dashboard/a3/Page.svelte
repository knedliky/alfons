<script lang="ts">
	 ...
	 */
	import { Footer, Header, LongreadStatBand, MainLayout, PageFrame, PageSection } from '@alfons/design';
	...
	$effect(() => {
		const pending: TransitionEvent[] = [
			{ taskId: 'GW-023', title: 'Token exchange for the agent fleet', from: 'pending', to: 'triaged', at: '14:31' },
			{ taskId: 'AL-011', title: 'Storybook: a story per component', from: 'building', to: 'verifying', at: '14:36' },
			{ taskId: 'LDG-052', title: 'Search the corpus by title fragment', from: 'building', to: 'verifying', at: '14:44' }
		];
		let index = 0;
...
<PageFrame>
	...
	>
		...
		<PageSection maxWidth="1600px">
			<MainLayout asideWidth="20rem" gap="xl" stickyTop="calc(var(--header-height) + var(--space-4))">
				{#snippet aside()}
					<ReleaseIndex
						{releases}
						tasks={liveTasks}
