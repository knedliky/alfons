		...
		<PageSection maxWidth="1440px">
			<Container maxWidth="full" padding={false}>
				<div class="masthead">
					...
					<h1 class="display">
						One hundred and forty-two tasks shipped in a fortnight, and one that has not moved since
						June
					</h1>
					<p class="standfirst">
						Eleven projects, dozens of releases and several hundred tasks, and the only thing anyone
						ever wants from it is where one particular piece of work stands. Start by remembering
						roughly what it was called.
					</p>
			...
			<Container maxWidth="full" padding={false}>
				<section class="search" aria-labelledby="search-heading">
					...
					<p class="section-deck">
						Search runs over titles, ids, releases and projects. Every result carries its release,
						its project and its phase, because a title alone is rarely enough to recognise the right
						one.
					</p>
					...
					<div class="results">
						...
						{#if results.length === 0}
							<p class="no-results">
								Nothing matches “{query}”. Titles in this corpus tend to read as sentences — try a
								word from the middle of one.
							</p>
						{/if}
					</div>
		...
		<PageSection maxWidth="1440px">
			<Container maxWidth="full" padding={false}>
				<section class="narrative" aria-labelledby="narrative-heading">
					...
					<p class="section-deck">
						Not a grid of equal cards. A release earns its place in the spread by what it is for,
						and the one holding the task you selected leads.
					</p>
							...
							<div class="entries">
								{#each orderedReleases as release, index (release.slug)}
									<ReleaseEntry
										...
										{selectedTaskId}
										{dependencies}
										onSelectTask={(id) => (selectedTaskId = selectedTaskId === id ? null : id)}
									/>
								{/each}
							</div>
						{/snippet}
						...
						{#snippet aside()}
							<div class="stop-press">
								...
								<p class="stop-press-deck">
									Status transitions as the ledger announces them. The rows above correct themselves
									as each one lands.
								</p>
								<ul class="feed">
